...
Alu.add_components(
    (Library.load('Mux16'), 6), #Mux de 0 a 6
    (Library.load('Not16'), 3), #Not16 7, 8 e 9
    Library.load('And16'), #And 10
    Library.load('Add16'), #Add 11
    Nor16way #Nor 12
)
Alu.set_as_input(1, 'sel', 'zx')
Alu.set_as_input(2, 'sel', 'nx')
Alu.set_as_input(3, 'sel', 'zy')
Alu.set_as_input(4, 'sel', 'ny')
Alu.set_as_input(5, 'sel', 'f')
Alu.set_as_input(6, 'sel', 'no')

for i in range(16):
    Alu.set_as_input(1, f'a{i}',f'x{i}')
    #Alu.set_as_input(1, f'b{i}',  )
    Alu.set_as_input(3, f'a{i}',f'y{i}')
    #Alu.set_as_input(3, f'b{i}', 0)
    Alu.connect(1, f'out{i}', 7, f'in{i}')
    Alu.connect(1, f'out{i}', 2, f'a{i}')
    Alu.connect(3, f'out{i}', 8, f'in{i}')
    Alu.connect(3, f'out{i}', 4, f'a{i}')
    Alu.connect(7, f'out{i}', 2, f'b{i}')
    Alu.connect(8, f'out{i}', 4, f'b{i}')
    Alu.connect(2, f'out{i}', 10, f'a{i}')
    Alu.connect(4, f'out{i}', 10, f'b{i}')
    Alu.connect(2, f'out{i}', 11, f'a{i}')
    Alu.connect(4, f'out{i}', 11, f'b{i}')
    Alu.connect(10, f'out{i}', 5, f'a{i}')
    Alu.connect(11, f'out{i}', 5, f'b{i}')
    Alu.connect(5, f'out{i}', 6, f'a{i}')
    Alu.connect(5, f'out{i}', 9, f'in{i}')
    Alu.connect(9, f'out{i}', 6, f'b{i}')
    Alu.connect(6, f'out{i}', 12, f'in{i}')
    Alu.set_as_output(6, f'out{i}', f'out{i}' )
Alu.set_as_output(12, 'out', 'zr' )
